...
/**
    ...
    @tags{Audio}
*/
class StretchPluginHolder    : private AudioIODeviceCallback,
                                  private Timer,
...
public:
    ...
    */
    StretchPluginHolder (PropertySet* settingsToUse,
    ...
    {
        // Only one StretchPluginHolder may be created at a time
    ...
    static StretchPluginHolder* getInstance()
    {
...
private:
    ...
    {
    public:
        SettingsComponent (StretchPluginHolder& pluginHolder,
...
public:
    ...
    StretchFilterWindow (const String& title,
                            ...
                            std::unique_ptr<StretchPluginHolder> pluginHolderIn)
          ...
          pluginHolder (std::move (pluginHolderIn))
    {
        ...
        setTitleBarHeight (0);
       #else
        // Native OS title bar. Done HERE in the wrapper ctor -- before any
        // content exists and while no hierarchy listeners can interfere --
        // so the peer is created with the native frame from the start
        // (see ERRATA.md: mid-flight switches glitch; editor-driven ones
        // froze the window at its 128px minimum).
    ...
    */
    StretchFilterWindow (const String& title,
        ...
        : StretchFilterWindow (title,
                                  backgroundColour,
                                  std::make_unique<StretchPluginHolder> (settingsToUse,
    ...
    // The JUCE "Options" title-bar button is gone (native title bar owns the
    // chrome now); audio settings are reachable from the plugin's hamburger
    // menu via StretchPluginHolder::getInstance()->showAudioSettingsDialog().

    virtual StretchPluginHolder* getPluginHolder()    { return pluginHolder.get(); }
...
private:
    ...
    {
    public:
        MainContentComponent (StretchFilterWindow& filterWindow)
